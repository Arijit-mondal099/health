...
export const userSignup = createAsyncThunk("user/userSignup", async (credentials, thunkAPI) => {
    try {
...
export const userAppointments = createAsyncThunk("user/userAppointments", async (_, thunkAPI) => {
    try {
        const token = localStorage.getItem("userToken");
...
export const cancelAppointment = createAsyncThunk(
    ...
    async (id, thunkAPI) => {
        try {
            ...
            const { data } = await axios.get(
                `${import.meta.env.VITE_BACKEND_URL}/api/v1/users/cancel-appointment/${id}`,
                { headers: { Authorization: `Bearer ${token}` } },
            );
