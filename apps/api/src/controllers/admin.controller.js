...
export const addDoctor = async (req, res) => {
    try {
        ...
        if (
            [name, email, password, speciality, degree, experience, about, fees, address].some(
                (field) => field?.trim() === "",
            )
        ) {
        ...
        if (!createdDoctor) {
            return res
                .status(400)
                .json(new ApiResponse(400, "Somthing went wrong while registring doctor!"));
        }

        return res
            .status(201)
            .json(new ApiResponse(201, "Doctor registered successfully!", createdDoctor));
    } catch (error) {
...
export const loginAdmin = async (req, res) => {
    try {
        ...
        if (!user) {
            return res
                .status(404)
                .json(new ApiResponse(404, "Admin with credential hasn't exist!"));
        }
...
export const getAllDoctors = async (req, res) => {
    try {
        ...
        return res
            .status(200)
            .json(new ApiResponse(200, "Successfully fetched all doctors", doctors));
    } catch (error) {
        return res.status(500).json(new ApiResponse(500, error?.message, error));
    }
};
...
export const dashboard = async (req, res) => {
    try {
        ...
        return res.status(200).json(new ApiResponse(200, "success", { doctor, user, appointment }));
    } catch (error) {
        return res.status(500).json(new ApiResponse(500, error?.message, error));
    }
};