...
export const registerUser = async (req, res) => {
  try {
    ...
    if (isUserExist) {
      return res.status(400).json(new ApiResponse(400, "User alredy exist with credintials!"));
    }
    ...
    if (!createdUser) {
      return res.status(400).json(new ApiResponse(400, "Somthing went wrong while registring user!"));
    }
    ...
    return res.status(201).json(new ApiResponse(201, "User registered successfully", userToken));
  } catch (error) {
...
export const loginUser = async (req, res) => {
  try {
    ...
    return res.status(200).json(new ApiResponse(200, "User registered successfully", userToken));
  } catch (error) {
    return res.status(500).json(new ApiResponse(500, error?.message, error));
  }
};
...
export const onlinePayment = async (req, res) => {
  try {
    ...
    if (!appointment || appointment.cancel) {
      return res.status(404).json(new ApiResponse(404, "Appointment has been canceled or not found!"));
    }
